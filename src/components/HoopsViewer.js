...
const HoopsViewer = (props) => {
    ...
    const [textarea_value, setCommentTextarea] = useState('');
    //  let [hwv, setHwv] = useState();
    let [isEventTriggered, setIsEventTriggered] = useState();
    let hwv = null;
    var token = 'eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJ3b29kd2FyZGJzbXVzZXIxIiwiZXhwIjoxNjQwOTY2ODUwLCJpYXQiOjE2NDA5MzA4NTB9.P0iVXA5VzhNCFtReTXruI4D1NvqH3sGK70JNNozL0ZA';
    ...
    var activeItem = {
        "_noteElementIsetNewCamd": "5e43a28d-0b64-4d1c-ada0-4d60362d6332",
        "_partId": 71,
        "_position": {
            x: 35.77206802368164,
            y: 36.40321731567383,
    ...
    useEffect(() => {
        if (props.clickedId !== null) {
            ...
            //    console.log("markupDataNew is", markupDataNew);
    ...
    const getCommentsByThread = (selectionPosition) => {
        fetch(`http://apriordevapp-env.eba-ajmpa4mr.us-east-1.elasticbeanstalk.com/api/ws/comment-threads/prequote-rfqs/${prequote_rfq_id}/comment-channels/${comment_channel_id}/anchor-location/${selectionPosition}/anchor-type/CELL`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
